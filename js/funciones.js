...
$(document).ready(function(){
	...
	$("#txtFecInicio").datepicker('option', 'dayNames', ['Domingo', 'Lunes', 'Martes', 'Mi�rcoles', 'Jueves', 'Viernes', 'S�bado'] );
	$("#txtFecInicio").datepicker('option', 'dayNamesMin', ['Do', 'Lu', 'Ma', 'Mi', 'Ju', 'Vi', 'S�'] );
	...
	$("#txtFecFinalizacion").datepicker('option', 'dayNames', ['Domingo', 'Lunes', 'Martes', 'Mi�rcoles', 'Jueves', 'Viernes', 'S�bado'] );
	$("#txtFecFinalizacion").datepicker('option', 'dayNamesMin', ['Do', 'Lu', 'Ma', 'Mi', 'Ju', 'Vi', 'S�'] );
...
function mostrarCuadroInfo(resp){
	...
	var tdCol1 = $("<td></td>").addClass("tdCampos").html("<b>Pedido:</b> N� " + resp.nroPedido);
	var tdSep1 = $("<td></td>").addClass("tdSep");
...
function procesarModificarPedido(resp){
	if(resp.estado == "ERR"){
		...
		alertar("El pedido se modific� correctamente");
		$("#infoGeneral").show();
		configurarLista("#infoGeneral");
		$("#infoGeneral").css("left",$("#divListadoPedido .borCentro").width() + 30);
		$("#infoGeneral").css("top",$("#divListadoPedido").position().top);
...
function eliminarPedido(id){
	if(confirm("�Est� seguro que desea eliminar el pedido?")){
		$.ajax({
			url: "procedimientos/pedidos.php",
			type: "POST",
			data: {
...
function procesarEliminarPedido(resp){
	if(resp.estado == "ERR"){
		...
		alertar("El pedido se elimin� correctamente");
		if(g_tipo == "N"){
			cargarPedidos(g_pedido_sel);
		}
	}
...
function modificarProducto(id, valor){
	var data = null;
	if(valor == null || valor == ""){
		if(!confirm("Vas a eliminar este producto, �Est�s seguro?")){
...
function procesarModificarProducto(resp){
	if(resp.estado == "ERR"){
		...
		alertar("El producto se modific� correctamente");
	}
	else if(resp.estado == "OKDel"){
		cargarListaProductos($("#cmbTipo").val());
		alertar("El producto se elimin� correctamente");
	}
	g_cantProc -= 1;
}
...
function procesarModificarCliente(resp){
	if(resp.estado == "ERR"){
		...
		alertar("El cliente se modific� correctamente");
	}
	else if(resp.estado == "OKDel"){
		cargarListaClientes();
		alertar("El cliente se elimin� correctamente");
	}
	g_cantProc -= 1;
}
...
function procesarAgregarMiProd(resp){
	if(resp.estado == "ERR"){
		...
		alertar("El producto se cre� correctamente");
